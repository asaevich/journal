...
class JournalController extends Controller
{
    ...
    {
        if (!$request->has(['semester_id', 'employment_type_id'])) {
    ...
    private function getSemesterDates(Semester $semester): array
    {
        ...
        foreach ($monthPeriod as $month) {
            ...
            foreach ($dateRange as $date) {
                $weeks[$weekNumber][$date->format('D')] = $date->format(/*'Y-m-d'*/'c');
    ...
    private function setTransfers($semesterLessons, Collection $timetable): array
    {
        ...
        foreach ($transfers as $transfer) {
            ...
            $oldDateLessons = $semesterLessons[$oldMonthNum][$transfer->old_date->format('c')];
            ...
            if (count($oldDateLessons) == 0) {
                unset($semesterLessons[$oldMonthNum][$transfer->old_date->format('c')]);
            }
            ...
            $semesterLessons[$newMonthNum][$transfer->new_date->format('c')][] = $transfer->lesson;
        }
    ...
    private function setReplacements($semesterLessons, Semester $semester, $lecturer): array
    {
        ...
        foreach ($meReplaced as $replacement) {
            ...
            $dateLessons = $semesterLessons[$lessonMonth][$replacement->lesson_date->format('c')];
            ...
            if (count($dateLessons) == 0) {
                unset($semesterLessons[$lessonMonth][$replacement->lesson_date->format('c')]);
            }
        }
        ...
        foreach ($iReplaced as $replacement) {
            ...
            $semesterLessons[$lessonMonthNum][$replacement->lesson_date->format('c')][] = $replacement->lesson;
        }
    ...
    private function createJournal(array $semesterLessons, $lecturer, string $name)
    {
        ...
        foreach ($semesterLessons as $monthNum => $monthLessons) {
            ...
            $oldLecturerName = implode(' ', [$lecturer->last_name, $lecturer->first_name, $lecturer->middle_name]);
            $worksheet->setCellValue('A1', $oldLecturerName);
            ...
            foreach ($monthLessons as $date => $lessons) {
                ...
                foreach ($lessons as $lesson) {
                    ...
                    // Если пара является переносом, то делаем соответствующую заметку
                    if (isset($lesson->old_date)) {
                        $str = "Перенос с " . $lesson->old_date->format('d.m.y');
                        $worksheet->setCellValueByColumnAndRow($lessonTypeColNum + 1, $groupRowNum, $str);
                    }

                    // Если пара является заменой, то делаем соответствующую заметку
                    if (isset($lesson->old_lecturer)) {
                        $oldLecturerName = implode(' ', [
                            $lesson->old_lecturer->last_name,
                            substr($lesson->old_lecturer->first_name, 0, 2) . '.',
                            substr($lesson->old_lecturer->middle_name, 0, 2) . '.'
                        ]);
                        $worksheet->setCellValueByColumnAndRow($lessonTypeColNum + 1, $groupRowNum,
                            "Замена $oldLecturerName");
                    }
            ...
            $oldLecturerName = $lecturer->last_name . ' '
                . substr($lecturer->first_name, 0, 2) . '. '
                . substr($lecturer->middle_name, 0, 2) . '.';
            $worksheet->setCellValue("A$lastRowNum", "Преподаватель $oldLecturerName");
